<?php 
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $return = [];
    $return['action'] = "approveBookReservation";
    ...
    if (!isset($_SESSION['staff_index'])) {
      ...
      $return['redirect'] = './staff-login.php';
        ...
        if ($hasReserveBool == "1") {
            $approveReservation = $con->prepare("DELETE FROM reserve WHERE reserve_index=:reserve_index");
            $approveReservation->bindParam(":reserve_index", $reserveIndex);
            $approveReservation->execute();
